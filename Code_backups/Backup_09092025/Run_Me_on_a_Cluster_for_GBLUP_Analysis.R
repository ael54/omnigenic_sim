...
these.prediction.accuracies.QTNs.GBLUP <- list()
these.prediction.accuracies.SNPs.GBLUP <- list()
    ...
    for(k in c(0.05,0.5,1,2)){
       for(el in c(0.05,0.5,1,2)){
         for(rep in 1:3){
           ...
           source("Master_GBLUP_Analysis_Script.R")
   
           ##############################################################
           #### Append GS Results
           #Append results to the lists you created before the start of this rep
           these.prediction.accuracies.QTNs.GBLUP[[count]] <- prediction.accuracies.QTNs 
           names(these.prediction.accuracies.QTNs.GBLUP)[count] <- this.setting 
           
           these.prediction.accuracies.SNPs.GBLUP[[count]] <- prediction.accuracies.SNPs
           names(these.prediction.accuracies.SNPs.GBLUP)[count] <- this.setting 
