...
these.prediction.accuracies.QTNs.Multi.Kernel.BLUP <- list()
these.prediction.accuracies.SNPs.Multi.Kernel.BLUP <- list()
    ...
    for(k in c(0.05,0.5,1,2)){
       for(el in c(0.05,0.5,1,2)){
         for(rep in 1:3){
           ...
           source("Master_Multi_Kernel_BLUP_Analysis_Script.R")
           
           ##############################################################
           #### Append GS Results
           #Append results to the lists you created before the start of this rep
           these.prediction.accuracies.QTNs.Multi.Kernel.BLUP[[count]] <- prediction.accuracies.QTNs 
           names(these.prediction.accuracies.QTNs.Multi.Kernel.BLUP)[count] <- this.setting 
           
           these.prediction.accuracies.SNPs.Multi.Kernel.BLUP[[count]] <- prediction.accuracies.SNPs
           names(these.prediction.accuracies.SNPs.Multi.Kernel.BLUP)[count] <- this.setting 
