...
these.prediction.accuracies.QTNs.Multi.Kernel.BLUP_ADD_and_EPI <- list()
these.prediction.accuracies.SNPs.Multi.Kernel.BLUP_ADD_and_EPI <- list()
    ...
    for(k in c(0.05,0.5,1,2)){
       for(el in c(0.05,0.5,1,2)){
         for(rep in 1:3){
           ...
           source("Master_Multi_Kernel_BLUP_Analysis_Script_ADD_and_EPI.R")
           
           ##############################################################
           #### Append GS Results
           #Append results to the lists you created before the start of this rep
           these.prediction.accuracies.QTNs.Multi.Kernel.BLUP_ADD_and_EPI[[count]] <- prediction.accuracies.QTNs 
           names(these.prediction.accuracies.QTNs.Multi.Kernel.BLUP_ADD_and_EPI)[count] <- this.setting 
           
           these.prediction.accuracies.SNPs.Multi.Kernel.BLUP_ADD_and_EPI[[count]] <- prediction.accuracies.SNPs
           names(these.prediction.accuracies.SNPs.Multi.Kernel.BLUP_ADD_and_EPI)[count] <- this.setting 
