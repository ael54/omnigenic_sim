...
simulate.omni.four.trait.trick <- function(input.SNPs = NULL,
  ...
  # of additive effects, followed by a single vector of epistatic effects
  custom.additive.for.simplePHENOTYPES <- list(trait_1 = c(additive.effects.of.core.genes, 
                                            additive.effects.of.peripheral.genes),
                                            trait_2 = c(additive.effects.of.core.genes,
                                                        rep(0, length(additive.effects.of.peripheral.genes))),
                                            trait_3 =  c(rep(0,length(additive.effects.of.core.genes)),
                                                         additive.effects.of.peripheral.genes),
                                            trait_4 = c(rep(0,length(additive.effects.of.core.genes)),
                                                        rep(0,length(additive.effects.of.peripheral.genes))))
  ...
  custom.epistatic.for.simplePHENOTYPES <- list(trait_1 = c(epistatic.effects.of.core.genes,
                                             ...
                                             trait_2 = c(epistatic.effects.of.core.genes,
                                                        rep(0, length(epistatic.effects.of.peripheral.genes)),
                                                        rep(0, length(epistatic.effects.between.core.and.perhpheral))),
                                             trait_3 = c(rep(0,length(epistatic.effects.of.core.genes)),
                                                         epistatic.effects.of.peripheral.genes,
                                                         rep(0, length(epistatic.effects.between.core.and.perhpheral))),
                                             trait_4 = c(rep(0,length(epistatic.effects.of.core.genes)),
                                                         rep(0,length(epistatic.effects.of.peripheral.genes)),
                                                         epistatic.effects.between.core.and.perhpheral))
  ...
  QTN_list$add[[2]] <- c(core.genes, peripheral.genes)
  QTN_list$add[[3]] <- c(core.genes, peripheral.genes)
  QTN_list$add[[4]] <- c(core.genes, peripheral.genes) 
  
  QTN_list$epi[[1]] <- c(core.pairwise.epistasis, peripheral.pairwise.epistasis,
                    core.peripheral.pairwise.epistasis)
  QTN_list$epi[[2]] <- c(core.pairwise.epistasis, peripheral.pairwise.epistasis,
                         core.peripheral.pairwise.epistasis)
  QTN_list$epi[[3]] <- c(core.pairwise.epistasis, peripheral.pairwise.epistasis,
                         core.peripheral.pairwise.epistasis)
  QTN_list$epi[[4]] <- c(core.pairwise.epistasis, peripheral.pairwise.epistasis,
                         core.peripheral.pairwise.epistasis)
  
  ####Tying in some loose ends: (these two lines are actually not used)
